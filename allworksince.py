...
# Build the JQL query based only on dates
jql = f"worklogDate >= '{display_date}'"
if end_date:
    ...
    jql = f"worklogDate >= '{display_date}' AND worklogDate < '{end_date_plus_one}'"
...
start_at = 0
max_results = 100
total = 1  # Initial value just to enter the loop
...
while start_at < total:
    ...
    # Update total and append issues from this page
    ...
    if args.debug:
        print(f"Retrieved {len(page_issues)} issues (total so far: {len(all_issues)} of {total})")
...
print(f"Fetching worklogs for {len(issues)} issues (this may take a while)...")
...
for i, issue in enumerate(issues, 1):
    # Print progress every 5 issues or for the last issue
    if i % 5 == 0 or i == total_issues:
        print(f"Processing issue {i}/{total_issues}...", end="\r")
    key = issue["key"]
    ...
    # Fetch worklogs with pagination
    worklog_url = f"{JIRA_SERVER}/rest/api/3/issue/{key}/worklog"
    
    # Initialize variables for worklog pagination
    all_worklogs_for_issue = []
    worklog_start_at = 0
    worklog_max_results = 100
    worklog_total = 1  # Initial value just to enter the loop
    
    # Fetch all worklogs with pagination
    while worklog_start_at < worklog_total:
        worklog_params = {
            "maxResults": worklog_max_results,
            "startAt": worklog_start_at
        }
        
        worklog_response = requests.get(worklog_url, headers=headers, params=worklog_params)
        worklog_data = worklog_response.json()
        
        # Update total and append worklogs from this page
        worklog_total = worklog_data.get("total", 0)
        page_worklogs = worklog_data.get("worklogs", [])
        all_worklogs_for_issue.extend(page_worklogs)
        
        # Move to next page
        worklog_start_at += worklog_max_results
        
        if args.debug and worklog_total > worklog_max_results:
            print(f"  Retrieved {len(page_worklogs)} worklogs for {key} (total: {len(all_worklogs_for_issue)} of {worklog_total})")
    
    worklogs = all_worklogs_for_issue

    for log in worklogs:
        # Get author information, falling back to alternatives if emailAddress is not available
        author_info = log.get("author", {})
        author = author_info.get("emailAddress", author_info.get("displayName", author_info.get("name", "Unknown User")))
        started = log["started"]
...
print("")
print(f"Successfully processed all {total_issues} issues and found {len(all_worklogs)} worklog entries")
...
print(f"{'Date':<12}{'Issue':<15}{'Epic':<15}{'Author':<25}{'Summary':<30}{'Time':<10}Comment")
print(f"{'-'*12}{'-'*15}{'-'*15}{'-'*25}{'-'*30}{'-'*10}{'-'*30}")
for log in sorted_worklogs:
    date = log["started_date"]
    epic = log.get("epic_link", "No Epic")
    author = log["author"]
    print(f"{date:<12}{log['key']:<15}{epic if epic else 'No Epic':<15}{author[:23]:<25}{log['summary'][:28]:<30}{log['time_spent']:<10}{log['comment_text']}")
...
for epic_id in epic_links:
    ...
    epic_author_times[epic_id] = {}

# Sum up time per epic and per author within epic
for log in all_worklogs:
    ...
    # Add to author within epic
    if author not in epic_author_times[epic_id]:
        epic_author_times[epic_id][author] = 0
    epic_author_times[epic_id][author] += minutes
...
for epic_id, minutes in sorted_epics:
    ...
    # Print time per author for this epic
    author_times = epic_author_times[epic_id]
    if author_times:
        # Sort authors by time spent
        sorted_authors = sorted(author_times.items(), key=lambda x: x[1], reverse=True)
        for author, author_minutes in sorted_authors:
            # Format author time
            author_hours = author_minutes // 60
            author_remaining_minutes = author_minutes % 60
            author_time_formatted = f"{author_hours}h"
            if author_remaining_minutes > 0:
                author_time_formatted += f" {author_remaining_minutes}m"
                
            print(f"{'  → ':<5}{author[:48]:<50}{author_time_formatted:<15}")
        print()
